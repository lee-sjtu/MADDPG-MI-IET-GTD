...
data_orig = pd.read_excel('E:\OneDrive - sjtu.edu.cn\【科研】\小论文\【3】IET-GTD\程序\EVdata1.xlsx')
...
class Scenario(BaseScenario):
    ...
    def ev_info(self, agent):
        ...
        if agent.fast: #如果是快充站
            # for i in range(agent.n_piles): #站内有多少充电桩，生成多少EV数据，不考虑排队
    ...
    def agents_state(self, t_arrive, t_leave, e_arrive, agent, world):
        ...
        for i in range(agent.n_piles):
            ...
            e_down_p[t_leave[i] - 5] = e_down_p[
                                           t_leave[i] - 4] - agent.max_power * world.ita / agent.cap
            if e_down_p[t_leave[i] - 5] < agent.min_soc:
                e_down_p[t_leave[i] - 5] = agent.min_soc
            ...
            for i, agent in enumerate(world.agents):
                ...
                if i < self.num_fag:
                    for j in range(agent.n_piles):
                        r_c.append((-1 * p_n[i][j] * station.state.price_b[station.state.t_cur]) / 5)
                else:
                    for j in range(agent.n_piles):
                        r_c.append((-1 * p_n[i][j] * station.state.price_b[station.state.t_cur]))

                rew_cost.append(r_c)

            if (station.p_ex_max) < abs(station.state.p_tot):
                rew_punish1 = (station.p_ex_max - abs(station.state.p_tot)) / 210 / 0.2
            else:
                rew_punish1 = 0
            # print(rew_cost, rew_punish1)
    ...
    def station_observation(self, world):
        for station in world.stations:
            ...
            #     eme_agent = 0
            # else:
            #     eme_agent = station.state.eme_tot/8
