...
class VDN:
    def __init__(self, args):
        ...
        if self.args.load_model:
            if os.path.exists(self.model_dir + '/' + '/power_li_rnn_net_params54.pkl'):
                path_rnn = self.model_dir + '/' + '/power_li_rnn_net_params54.pkl'
                path_vdn = self.model_dir + '/' + '/power_li_vdn_net_params54.pkl'
    ...
    def learn(self, batch, max_episode_len, train_step, epsilon=None):  # train_step表示是第几次学习，用来控制更新target_net网络的参数
        ...
        if train_step > 0 and train_step % self.args.target_update_cycle == 0:
            # if train_step > 0 and train_step % (self.args.target_update_cycle*2) == 0:
                # print('target network parameters updated')
            self.target_rnn.load_state_dict(self.eval_rnn.state_dict())
            self.target_vdn_net.load_state_dict(self.eval_vdn_net.state_dict())
        return loss
    ...
    def save_model(self, train_step):
        ...
        print('model', num, 'saved')
        torch.save(self.eval_vdn_net.state_dict(), self.model_dir + '/' + num + '_power_li_vdn_net_params54-0824.pkl')
        torch.save(self.eval_rnn.state_dict(),  self.model_dir + '/' + num + '_power_li_rnn_net_params54-0824.pkl')
