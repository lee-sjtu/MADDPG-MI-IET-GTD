...
def parse_args():
    ...
    parser.add_argument("--num-ncp", type=int, default=20, help="number of norm charge piles")
    parser.add_argument("--num-fcp", type=int, default=10, help="number of fast charge piles")
...
def train(arglist):
    with tf.Session() as sess:
        ...
        while True:
            ...
            if terminal:
                if tot_step % 100 == 0:
                    print(tot_step, time.perf_counter())
                obs_n, soc_, e_up, e_down, t_stay_, num_ev = env.reset()
                episode_step = 0
                if tot_step < arglist.num_episodes:
                    ...
                    for a in agent_p1_rewards:
                        a.append(0)
                if tot_step > 1000 and tot_step % 1000 == 0:
                    plotma(
                        agent_tot_rewards[0], agent_tot_rewards[1],
                        agent_c_rewards[0], agent_c_rewards[1],
                        agent_p1_rewards[0], agent_p1_rewards[0],
                        agent_tot_loss[0], agent_tot_loss[1])
                tot_step += 1
                # if noise > 0.1 and tot_step > 1000:  # tot_step > arglist.memory_size
                #     noise *= 0.995
            if tot_step > arglist.num_episodes:
        ...
        saver.save(sess, './save_pl_mi_20+10/params', write_meta_graph=False)
