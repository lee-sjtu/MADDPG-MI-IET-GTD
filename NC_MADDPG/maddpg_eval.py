...
def parse_args():
    ...
    parser.add_argument("--num-norm-piles", type=int, default=5, help="number of norm charge piles")
...
def make_env(scenario_name, num_fast_agents, num_norm_piles, num_EVs):
    from environment_eval import MultiAgentEnv
    import scenarios as scenarios
    # load scenario from script
    scenario = scenarios.load(scenario_name + ".py").Scenario()
    # create world
    world = scenario.make_world(num_fast_agents, num_norm_piles, num_EVs)
...
def maddpg_eval(arglist, env):
    with tf.Session() as sess:
        ...
        saver.restore(sess, './save17-MA-5EV-poli-08061/params')
        ...
        while True:
            ...
            for i in range(arglist.num_EVs_per_agent):
                ...
                #print("soc0", obs_n[i][0][0], "soc1", obs_n[i][1][0])  # , "soc2", obs_n[2][0])
            # print(action_N)
            for i in range(arglist.num_EVs_per_agent):
            ...
            # print("2", soc)
            new_obs_n, p_n, cost = env.step(action_N, obs_n, arglist.num_EVs_per_agent, soc_, e_up, e_down, t_stay_)# , rew_n,  rc_n, rp_n
            #print("power0", p_n[0], "power1", p_n[1], "total", sum(p_n))  # , "rp_n", rp_n
            COST += cost
            print("1", p_n, sum(sum(p_n)))
            # print(p_n, "total", sum(p_n))
            for i in range(arglist.num_EVs_per_agent):
                for j in range(len(p_n[i])):
                    POWER_t.append(p_n[i][j])
                    # SOC_t.append(soc[i][j])
            POWER.append(POWER_t)
            SOC.append(soc)
            MAX_P_t.append(sum(sum(p_n)))
...
if __name__ == '__main__':
    ...
    #-------------------------write output---------------------------#
    # writer = pd.ExcelWriter('/Users/lihang/Desktop/EV_result.xlsx')
    writer = pd.ExcelWriter('.\save17-MA-5EV-poli-08061.xlsx')
