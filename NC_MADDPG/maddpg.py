...
def p_train(make_obs_ph_n, make_act_ph_n, num_piles, p_index, p_func, q_func, num_units1, num_units2, num_units3, optimizer, scope="trainer", reuse=None):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        for i in range(num_piles):
            ...
            act_input_n[p_index * num_piles + i] = p
            target_p = p_func(p_input, 1, num_units1, num_units2, num_units3, scope="target_p_func", reuse=tf.AUTO_REUSE)
            target_act.append(U.function(inputs=[obs_ph_n[p_index * num_piles + i]], outputs=target_p))
            p_values.append(U.function([obs_ph_n[p_index * num_piles + i]], p))
        # print("2", act_ph_n)
...
def q_train(make_obs_ph_n, make_act_ph_n, num_piles, q_func, num_units1, num_units2, num_units3, optimizer, scope="trainer", reuse=None):
    with tf.variable_scope(scope, reuse=reuse):
        obs_ph_n = make_obs_ph_n
        act_ph_n = make_act_ph_n
...
class MADDPGAgentTrainer(AgentTrainer):
    def __init__(self, name, model_a, model_c, obs_shape_n, act_space_n, agent_index, args):
        self.name = name
    ...
    def preupdate(self):
        self.replay_sample_index1 = None
        # self.replay_sample_index2 = None

    def update(self, agents, agent_index):
        self.replay_sample_index1 = self.replay_buffer.make_index1(int(self.args.batch_size))
        # self.replay_sample_index2 = self.replay_buffer.make_index2(12)#int(self.args.batch_size*0.25))
        ...
        # print(np.shape(rew_n))
        # print(done_n)
        obs_n = np.transpose(obs_n, (2, 1, 0, 3))
        act_n = np.transpose(act_n, (2, 1, 0, 3))
        rew_n = np.transpose(rew_n, (2, 1, 0))
        obs_next_n = np.transpose(obs_next_n, (2, 1, 0, 3))
        # done_n = np.transpose(done_n, (1, 0))
        # print(np.shape(rew_n))
        obs_N = []
        act_N = []
        obs_N_next = []
        for i in range(self.n):
            for j in range(self.args.num_EVs_per_agent):
        ...
        # for i in range(self.n):
        #     for j in range(self.args.num_EVs_per_agent):
        #         target_act_next_n.append(agents[i].p_debug['target_act'][j](*([obs_N_next[i * self.args.num_EVs_per_agent + j]]))) #+ np.random.normal(0, 0.1)
        # print(np.shape(obs_N_next))
        # print(np.shape(target_act_next_n))
        # print(obs_N_next + target_act_next_n)
        # num_sample = 1
        target_q = 0.0
        ...
        for i in range(self.n):
            for j in range(self.args.num_EVs_per_agent):
                target_act_next_n.append(agents[i].p_debug['target_act'][j](
                    *([obs_N_next[i * self.args.num_EVs_per_agent + j]])))  # + np.random.normal(0, 0.1)
        target_q_next = self.q_debug['target_q_values'](*(obs_N_next + target_act_next_n))
            # print(rew_n[agent_index], done_n)
        for k in range(self.args.num_EVs_per_agent):
            # print("1", agent_index, k, rew_n[agent_index][k])
            target_q += rew_n[agent_index][k] + self.args.gamma * (1.0 - done_n) * target_q_next
        target_q /= self.args.num_EVs_per_agent
