...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world, num_EVs_per_agent):
        ...
        for i in range(num_EVs_per_agent):
            ...
            # for j in range(int(len(arrive)/num_EVs_per_agent)):
            #     t_arrive.append(int(arrive[i + j]))
            #     t_leave.append(int(leave[i + j]))
            #     e_arrive.append(earrive[i + j])
            # t_arrive = arrive.reshape(num_EVs_per_agent, -1)[i]
            # t_leave = leave.reshape(num_EVs_per_agent, -1)[i]
            # e_arrive = earrive.reshape(num_EVs_per_agent, -1)[i]
            print("agnt", t_arrive, t_leave, e_arrive)
            ...
            for i in range(len(t_arrive)):
                t_arrive_n.append(t_arrive[i])
                t_leave_n.append(t_leave[i])
                e_arrive_n.append(e_arrive[i])
            t_stay.append(ag_t_stay)
            self.station_state(world)
        # EVinfo = [t_arrive_n, t_leave_n, e_arrive_n]
        # print(EVinfo)
        return soc_, e_up, e_down, t_stay, t_arrive_n, t_leave_n, e_arrive_n
    ...
    def agents_state(self, t_arrive, t_leave, e_arrive, world):
        ...
        for i, agent in enumerate(world.agents):
            ...
            agent.state.t_stay[int(t_arrive[i])] = int(t_leave[i]) - int(t_arrive[i])
            # print("t_stay", agent.state.t_stay[t_arrive[i]])
            agent.state.soc[int(t_arrive[i])] = e_arrive[i]
            agent.state.e_up[int(t_arrive[i])] = e_arrive[i]
            agent.state.e_down[int(t_arrive[i])] = e_arrive[i]
            t_need = math.ceil((agent.soc_need-agent.min_soc) * agent.cap / agent.max_power / world.ita)
            # print("t_need", t_need)
            for t in range(int(t_arrive[i])+1, int(t_leave[i])+1):
                # ----------time before leave-----------
            ...
            agent.state.e_down[int(t_leave[i])] = agent.soc_need
            agent.state.e_down[int(t_leave[i]) - 1] = agent.soc_need - agent.max_power * world.ita / agent.cap
            agent.state.e_down[int(t_leave[i]) - 2] = agent.state.e_down[
                                                     int(t_leave[i]) - 1] - agent.max_power * world.ita / agent.cap
            agent.state.e_down[int(t_leave[i]) - 3] = agent.state.e_down[
                                                     int(t_leave[i]) - 2] - agent.max_power * world.ita / agent.cap
            if agent.state.e_down[int(t_leave[i]) - 3] < agent.min_soc:
                agent.state.e_down[int(t_leave[i]) - 3] = agent.min_soc
            agent.state.e_down[int(t_leave[i]) - 4] = agent.state.e_down[
                                                     int(t_leave[i]) - 3] - agent.max_power * world.ita / agent.cap
            if agent.state.e_down[int(t_leave[i]) - 4] < agent.min_soc:
                agent.state.e_down[int(t_leave[i]) - 4] = agent.min_soc
    ...
    def reward(self, world, p_n):  # reward for a particular agent
        for station in world.stations:
            ...
            for i in range(len(p_n)):
                cost += (-1 * p_n[i] * station.state.price_b[station.state.t_cur])
            return cost
    ...
    def station_observation(self, world, agent_index, num_EVs_per_agent):
        for station in world.stations:

            eme_agent = 0
            t = station.state.t_cur
            # for i, agent in enumerate(world.agents):
            #     if agent.state.n_ev == 0:
            #         eme_agent = 0
            #     else:
            ...
            sta_state = [[t / 24] + [station.state.price_b[t]] + [eme_agent]][0]# + num_evs_agent + eme_agent][0]

            return np.array(sta_state)
